...
#DEV
MYSQL_DB_URL="mysql://root:P@ssw0rd2022!!@tcp(103.134.154.73:6033)/mysql-orm?multiStatements=true"
...
pg.migrate.up: check-migrations-cmd
	migrate -path $(PG_MIGRATIONS_FOLDER) -database $(MYSQL_DB_URL) --verbose up

pg.migrate.down: check-migrations-cmd
	migrate -path $(PG_MIGRATIONS_FOLDER) -database $(MYSQL_DB_URL) --verbose down

pg.migrate.up.schema:
	go run main.go -schema=$(schema) -folder=$(folder)

pg.migrate.fix: check-migrations-cmd
	migrate -path $(PG_MIGRATIONS_FOLDER) -database $(MYSQL_DB_URL) force $(version)
